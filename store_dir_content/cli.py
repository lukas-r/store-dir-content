"""
This module provides a command-line interface (CLI) for performing various file operations,
including reading files from specified directories and writing their contents to an output file.
It supports filtering files based on their extensions, ignoring specific folders,
and respecting .gitignore files to exclude certain files.

Functions:
    read_files_in_directories: Reads files from the specified directories
    and writes their contents to an output file.

How to use it:
    The module can be run as a script or imported into other Python programs
    to utilize its functions. When run as a script, users can specify directories, file extensions,
    the output file, folders to ignore, and the option to respect .gitignore files
